...
void broadcast(/* const */ COMM& comm, const int root, /* const */ vector<T>& data)
{
	const int sz = data.size() * sizeof(typename vector<T>::value_type);
	comm.Bcast( &data[0], sz, MPI::CHAR, root );
}

template <class COMM, typename T>
void send(/* const */ COMM& comm, const int to, /* const */ vector<T>& data)
{
	const int sz = data.size() * sizeof(typename vector<T>::value_type);
	comm.Send( &data[0], sz, MPI::CHAR, to, TAG );
}

template <class COMM, typename T>
void receive(/* const */ COMM& comm, const int from, 
	/* const */ vector<T>& data, const int offset, const int size)
{
	const int sz = size * sizeof(typename vector<T>::value_type);
	comm.Recv( &data[offset], sz, MPI::CHAR, from, TAG );
}
...
{
	printf("\n--------------------------------\n");
	printf("Error - Usage: matrix_mult <matrix size>\n");
	printf("--------------------------------\n\n");
}
...
int main(int argc, char* argv[])
{
	...
	// distribute the data
	broadcast(MPI::COMM_WORLD, 0, mat_a);
	broadcast(MPI::COMM_WORLD, 0, mat_b);
	...
	if(my_rank == 0)
	{
		...
		if(partition_size != 0)
		{
			...
			for(int p = 1; p < num_procs; ++p)
			{
				...
				receive(MPI::COMM_WORLD, p, mat_c, offset, partition_size);
			}
		}
	...
	else
	{
		...
		if(partition_size != 0)
		{
			...
			// send to root
			send(MPI::COMM_WORLD, 0, mat_c_section);
		}
	}
