 ...
 */
   ...
   #undef Kb
#endif
static void ATL_sJIK0x0x72NT3x1x4_aX_bX
   (const int M, const int N, const int K, const float alpha, const float * ATL_RESTRICT A, const int lda, const float * ATL_RESTRICT B, const int ldb, const float beta, float * ATL_RESTRICT C, const int ldc)
/*
 * matmul with TA=N, TB=T, MB=0, NB=0, KB=72, 
...
#endif
#endif
static void ATL_sJIK0x0x72NT1x2x4_aX_bX
   (const int M, const int N, const int K, const float alpha, const float * ATL_RESTRICT A, const int lda, const float * ATL_RESTRICT B, const int ldb, const float beta, float * ATL_RESTRICT C, const int ldc)
/*
 * matmul with TA=N, TB=T, MB=0, NB=0, KB=72, 
 ...
 */
   ...
   if (k=N-Nb)
      ATL_sJIK0x0x72NT1x1x4_aX_bX(M, k, 72, alpha, ca, lda, cb + (Nb), ldb, beta, cc + (Nb*ldc), ldc);
}
#ifdef incAm
   #undef incAm
#endif
 ...
 */
   ...
   if (k=N-Nb)
      ATL_sJIK0x0x72NT3x1x4_aX_bX(M, k, 72, alpha, ca, lda, cb + (Nb), ldb, beta, cc + (Nb*ldc), ldc);
   if (Nb && (k=M-Mb))
      ATL_sJIK0x0x72NT1x2x4_aX_bX(k, Nb, 72, alpha, ca + (Mb), lda, cb, ldb, beta, cc + (Mb), ldc);
}
#ifdef incAm
   #undef incAm
#endif
